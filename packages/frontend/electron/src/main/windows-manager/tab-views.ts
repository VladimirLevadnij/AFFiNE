...
export class WebContentViewsManager {
  ...
  set activeWorkbenchId(id: string) {
  ...
  get activeWorkbenchId() {
    return (
      this.tabViewsMeta.activeWorkbenchId ?? this.tabViewsMeta.workbenches[0].id
    );
  }
  ...
  reorderViews = () => {
    if (this.mainWindow) {
      ...
      const ready = this.appTabsUIReady$.value.has(this.activeWorkbenchId);
  ...
  private readonly createAndAddView = async (
    ...
    view.webContents.on('destroyed', () => {
      ...
      // if all views are destroyed, close the main window
